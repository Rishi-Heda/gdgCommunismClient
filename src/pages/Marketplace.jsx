...
const Marketplace = () => {
  ...
  const categories = ['All Assets', 'Compute', 'Datasets', 'Neural Weights', 'Partner Rewards'];
    ...
    <div className="animate-fade-in pb-20">
      ...
      <div className="relative -mt-12 mb-24">
        ...
        <div className="flex flex-col md:flex-row justify-between items-end gap-6 px-4">
          ...
          <div className="flex gap-2 p-1 bg-surface-card border border-black/10 rounded-2xl">
            {categories.map((cat) => (
              <button
                key={cat}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        {/* Exchange Center */}
        <div className="lg:col-span-2 p-10 rounded-[40px] bg-black inverted-dark-container shadow-2xl relative overflow-hidden group">
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-mono uppercase tracking-[0.2em] mb-4 opacity-70">You Give (Hive Coins)</label>
                  <div className="relative">
                    ...
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 flex flex-col items-end">
                      <span className="text-xs font-mono font-bold">HC</span>
                      <span className="text-[10px] opacity-50 underline cursor-pointer" onClick={() => setExchangeAmount(mockWealth.hiveCoins)}>MAX</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-mono uppercase tracking-[0.2em] mb-4 opacity-70">You Receive (Mind Credits)</label>
                  <div className="bg-accent-primary/5 border-2 border-dashed border-accent-primary/20 rounded-3xl px-8 py-6 flex justify-between items-center">
                    <span className="text-4xl font-black">{mcReceive.toLocaleString()}</span>
                    <span className="text-sm font-mono font-bold">MC</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-accent-primary/5 rounded-[32px] p-8 border border-accent-primary/10 h-full flex flex-col justify-between">
                <div>
                  <h3 className="font-bold flex items-center gap-2 mb-4 text-text-primary">
                    <Zap className="w-4 h-4" /> Exchange Info
                  </h3>
                  <div className="space-y-4 text-sm font-medium">
                    <div className="flex justify-between border-b border-accent-primary/10 pb-2">
                      <span className="opacity-60">Rate</span>
                      <span>10 HC : 1 MC</span>
                    </div>
                    <div className="flex justify-between border-b border-accent-primary/10 pb-2">
                      <span className="opacity-60">Fee</span>
                      <span className="text-status-green">0.00%</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="opacity-60">Processing</span>
                      <span className="font-mono text-xs">INSTANT</span>
                    </div>
                  </div>
                </div>

                <button className="w-full bg-accent-primary text-black py-5 rounded-2xl font-black uppercase tracking-widest text-sm hover:scale-[1.02] active:scale-95 transition-all mt-8">
                  Confirm Exchange
                </button>
              </div>
            </div>
        ...
        <div className="p-8 rounded-[40px] bg-surface-card border border-black/10 flex flex-col gap-8">
          ...
          <div className="flex-1 bg-black/5 rounded-3xl p-6 border border-dashed border-black/20 flex flex-col items-center justify-center text-center">
            <Gift className="w-8 h-8 mb-3 text-text-muted" />
            <p className="text-xs font-mono uppercase tracking-widest text-text-muted">New Partner Rewards Coming Soon</p>
          </div>
        </div>
      ...
      <div className="mt-16">
        <div className="flex items-center justify-between mb-8 px-4">
          ...
          <button className="text-[10px] font-mono font-bold uppercase tracking-widest flex items-center gap-2 hover:text-accent-primary transition-colors">
          ...
          {partnerItems.map((item) => (
            <div key={item.id} className="bg-white border-2 border-black/5 rounded-[40px] p-8 hover:border-black transition-all cursor-pointer group h-full flex flex-col">
              <div className="flex justify-between items-start mb-6">
                <div className={`p-4 rounded-3xl ${item.company === 'NVIDIA' ? 'bg-black text-accent-primary' : 'bg-black text-white'}`}>
                  <item.icon className="w-6 h-6" />
                </div>
                <div className="text-[10px] font-mono font-bold uppercase tracking-widest px-3 py-1 rounded-full border border-black/10">
                  {item.company}
                </div>
              </div>
              ...
              <div className="flex items-center justify-between pt-6 border-t border-black/5">
                ...
                <button className="w-10 h-10 rounded-full bg-black text-accent-primary flex items-center justify-center group-hover:scale-110 transition-all">
